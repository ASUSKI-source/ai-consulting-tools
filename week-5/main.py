...
import anthropic
from rag_pipeline import index_document, ask_document, list_collections, CHROMA_CLIENT
...
def upload_document(
    ...
    try:
        ...
        try:
            ...
            index_info = index_document(tmp_path)
        finally:
        ...
        # Upsert document record so re-uploading the same collection updates metadata.
        try:
            existing = (
                db.query(Document)
...
def delete_document(
    ...
    try:
        # Delete the ChromaDB collection if it exists. Errors are swallowed so
        # that DB cleanup can still proceed.
        try:
