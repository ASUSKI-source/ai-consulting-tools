...
def get_user_message(error: Exception, context: str = '') -> dict:
    ...
    if 'incorrect password' in err_str or 'invalid credentials' in err_str:
        return {
    ...
    if 'token expired' in err_str or 'signature has expired' in err_str:
        return {
            'category': ErrorCategory.AUTH,
    ...
    if 'email already registered' in err_str or 'unique constraint' in err_str:
        return {
            'category': ErrorCategory.INVALID_INPUT,
            'user_message': 'That email is already registered.',
            'suggestion': 'Log in or use a different email.',
            'technical': str(error),
        }
