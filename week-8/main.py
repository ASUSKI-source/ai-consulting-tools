...
def _seed_guest_user_sync():
    """Run in a thread so startup does not block the event loop. Session is closed when done."""
    gen = get_db()
    db = next(gen)
    try:
        guest = db.query(User).filter(User.email == "guest@demo.com").first()
        if not guest:
            db.add(User(
...
async def seed_guest_user():
    await asyncio.to_thread(_seed_guest_user_sync)
...
@app.get('/login')
def serve_login():
    """Serves the standalone login/register page."""
...
def register(request: UserRegister, db: Session = Depends(get_db)):
    """Create a new user account. Does not return a token — user must log in explicitly."""
    email = request.email.lower().strip()
    existing = db.query(User).filter(User.email == email).first()
    if existing:
        info = get_user_message(ValueError('email already registered'))
        raise HTTPException(
            status_code=400,
            detail={'user_message': info['user_message'], 'suggestion': info['suggestion'], 'category': info['category'].value},
        )
...
def login(request: UserLogin, db: Session = Depends(get_db)):
    """Authenticate with email and password; return access token and user info."""
    email = request.email.lower().strip()
...
def auth_me(current_user: User = Depends(get_current_user)):
    """Return the current user's profile. Used by the frontend to verify the token on page load."""
    return UserResponse(
        id=current_user.id,
...
def logout():
    """Acknowledge logout. JWTs are invalidated client-side by removing the token from localStorage."""
...
def guest_login(db: Session = Depends(get_db)):
    ...
    return {
        "access_token": token,
        "token_type": "bearer",
        "user": {"id": guest.id, "email": guest.email, "full_name": "Guest User"}
    }
...
def get_stats(
    ...
    base = db.query(Analysis).filter(Analysis.user_id == current_user.id)
    total_analyses = base.count()
    stock_analyses = base.filter(Analysis.asset_type == 'stock').count()
    crypto_analyses = base.filter(Analysis.asset_type == 'crypto').count()

    unique_tickers = (
        base.with_entities(func.count(func.distinct(Analysis.ticker))).scalar() or 0
    )

    total_cost_raw = base.with_entities(func.sum(Analysis.estimated_cost)).scalar() or 0.0
    total_cost = round(float(total_cost_raw), 4)

    most_row = (
        base.with_entities(Analysis.ticker, func.count(Analysis.id).label('c'))
...
def analyze_stock_endpoint(
    ...
    try:
        ...
        try:
            ...
            db_analysis.user_id = current_user.id
            db.add(db_analysis)
            db.commit()
...
def get_history(
    ...
    query = db.query(Analysis).filter(Analysis.user_id == current_user.id).order_by(Analysis.created_at.desc())
